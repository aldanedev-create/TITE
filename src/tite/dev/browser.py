...
class DevServer:
    ...
    def __init__(
        ...
        # Get settings
        self.host = self.config["dev"]["host"]
        self.port = self.config["dev"]["port"]
        self.command = self.config["dev"]["command"]
        ...
        self.watcher = FileWatcher(
            paths=self.config.get("watcher", {}).get("paths", ["src", "tests"]),
            extensions=self.config.get("watcher", {}).get("extensions", [".py"]),
            ignore=self.config.get("watcher", {}).get("ignore", []),
            debounce=self.config.get("watcher", {}).get("debounce", 100),
        )

        self.reloader = Reloader(
            restart_on_change=self.config.get("watcher", {}).get("restart_on_change", True),
        )

        # Bind reloader hooks to runner execution
        self.reloader.on_pre_reload(lambda: self.runner.stop())
        self.reloader.on_reload(lambda: self._start_process())
        ...
        try:
            ...
            # Handles non-main thread execution or OS incompatibilities
            pass
    ...
    def start(self) -> None:
        ...
        os.environ["TITE_HOST"] = self.host
        ...
        try:
            while self.running:
                ...
                # Restart if auto-reload is enabled and server is active
                if not self.no_reload and self.running:
                    logger.info("Process exited unexpectedly, restarting...")
                    self._start_process()
                    time.sleep(1.0)  # Brief pause to prevent fast crash loops
    ...
    def _start_watcher(self) -> None:
        ...
        def on_change(events: List[Dict[str, Any]]):
            ...
            # Route change event through Reloader instance
